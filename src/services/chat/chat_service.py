...
class ChatService:
    ...
    def create_user_message(self, session, content: str, chat_id: UUID):
        last_message = self.__repo.get_last_message_in_chat(session, chat_id)
        sequence = last_message.sequence + 1 if last_message else 1

        message = Message(role="user", content=content, sequence=sequence, chat_id=chat_id)
        return self.__repo.save_message(session, message)

    def create_chat(self, session, title: str):
